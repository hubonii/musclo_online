...
async function seed() {
    try {
        console.log('--- STARTING MULTI-POSE PROGRESS PHOTO SEEDING ---');

        const user = await User.findOne({ where: { email: 'huboony@gmail.com' } });
        if (!user) throw new Error('User not found');
        const userId = user.id;

        console.log('Clearing old photos and history...');
        ...
        const prog = await Program.create({ user_id: userId, name: 'Evolution: PPL Mastery', description: 'Advanced 6-day split with full progress tracking.' });

        const pull = await Routine.create({ user_id: userId, program_id: prog.id, name: 'Pull Day', day_of_week: '1' });
        const push = await Routine.create({ user_id: userId, program_id: prog.id, name: 'Push Day', day_of_week: '2' });
        const legs = await Routine.create({ user_id: userId, program_id: prog.id, name: 'Leg Day', day_of_week: '3' });
        const swim = await Routine.create({ user_id: userId, program_id: prog.id, name: 'Swimming', day_of_week: '7' });
        ...
        let blobIdx = 0;
        console.log('Seeding 6 weeks with Front, Side, and Back photos...');
        ...
        for (let d = 0; d < 42; d++) {
            ...
            if (d % 7 === 0) {
                const m = await Measurement.create({ user_id: userId, date: ds, weight_kg: 90 - (d/7)*0.5, height_cm: 164 });
                
                // Add Front, Side, and Back photos for EACH measurement
                ...
                for (const pose of poses) {
                    ...
                    blobIdx++; // Increment to ensure the next pose/week gets a NEW photo
                }
            }
            ...
            const w = await WorkoutLog.create({
                ...
                started_at: `${ds}T18:00:00Z`, completed_at: `${ds}T19:30:00Z`, total_volume: type === 'cardio' ? 0 : 5000 + (d*200)
            });

            if (type !== 'cardio') {
                const res = await RoutineExercise.findAll({ where: { routine_id: r.id } });
                for (const re of res) {
                    let base = type === 'push' ? (re.sort_order === 0 ? 50 : 35) : type === 'pull' ? 45 : 70;
