...
async function seed() {
    try {
        console.log('--- STARTING PROPER COMPREHENSIVE PPL SEEDING ---');
        ...
        // Fetching more exercises for a "Proper" PPL
        ...
        console.log('Building Professional Program...');
        ...
        let bIdx = 0;
        console.log('Generating 6 weeks of high-fidelity history...');
        ...
        for (let d = 0; d < 42; d++) {
            ...
            if (d % 7 === 0) {
                const m = await Measurement.create({ user_id: userId, date: ds, weight_kg: 90 - (d/7)*0.6, height_cm: 164 });
            ...
            const workout = await WorkoutLog.create({
                ...
                started_at: `${ds}T18:00:00Z`, completed_at: `${ds}T19:45:00Z`, duration_seconds: 6300,
                total_volume: type === 'cardio' ? 0 : 6000 + (d*300)
            });

            if (type !== 'cardio') {
                const res = await RoutineExercise.findAll({ where: { routine_id: r.id } });
                for (const re of res) {
                    let base = 20; // Accessory weight
