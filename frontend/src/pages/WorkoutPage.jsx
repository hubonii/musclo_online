        ...
        return (
            <div className="p-4 md:p-8 max-w-3xl mx-auto flex flex-col items-center justify-center min-h-[60vh] text-center">
                <motion.div {...MOTION.pageEnter} className="space-y-6">
                    <div className="w-20 h-20 bg-primary/10 text-primary rounded-[32px] flex items-center justify-center mx-auto mb-6 shadow-neu-sm border border-primary/20">
                    ...
                    {isLoadingRoutine ? (
                        ...
                        <div className="space-y-6">
                            ...
                            <Button size="lg" variant="primary" onClick={handleStartWorkout} className="w-full sm:w-auto px-12 py-7 text-xl shadow-neu-primary border-4 border-white/20">
                                START WORKOUT
                            </Button>
                        </div>
                    ) : (
                        <div className="w-full flex justify-center items-center flex-col pt-4">
                            <h1 className="text-4xl font-black text-text-primary tracking-tighter uppercase">Workout Log</h1>
                            <p className="text-text-secondary max-w-sm mb-10 mx-auto font-medium">Choose a routine or start a quick session.</p>

                            <Button size="lg" variant="primary" onClick={handleStartWorkout} className="w-full sm:w-auto px-12 py-6 text-lg shadow-neu-primary border-4 border-white/20 mb-12">
                                ...
                                {isLoadingHistory ? (
                                    ...
                                    <div className="space-y-4">
                                        {recentRoutines.map((routine) => (
                                            <Card key={routine.id} className="p-5 flex items-center justify-between cursor-pointer hover:bg-primary/5 group shadow-neu-sm" onClick={() => navigate(`/workout/${routine.id}`)}>
                                                <div>
                                                    <h4 className="font-extrabold text-text-primary group-hover:text-primary transition-colors uppercase tracking-tight">{routine.name}</h4>
                                                    <p className="text-[10px] text-text-muted mt-1 font-bold uppercase tracking-widest">Routine</p>
                                                </div>
                                                <div className="w-12 h-12 rounded-2xl bg-app shadow-neu text-text-muted group-hover:text-primary flex items-center justify-center transition-all group-hover:shadow-neu-inset">
        ...
        <div className="flex flex-col h-full bg-app">
            ...
            <div className="flex-1 overflow-y-auto w-full pt-4 md:pt-8 pb-32">
                <div className="px-4 md:px-8 space-y-6 max-w-4xl mx-auto w-full">
                    ...
                    <motion.div>
                        <Button variant="outline" className="w-full py-8 border-dashed border-2 hover:border-solid text-text-secondary hover:text-primary hover:border-primary bg-app/50 shadow-neu-inset" icon={<Plus size={20}/>} onClick={() => setIsPickerOpen(true)}>
                            ADD EXERCISE
                        </Button>
                    </motion.div>
                    ...
                    <motion.div>
                        <Card className="shadow-neu-sm">
                            ...
                            <Textarea placeholder="How was your workout? Any notes on performance, feelings, or adjustments..." value={notes} onChange={handleSetNotes} className="min-h-[100px] bg-app shadow-neu-inset border-0 focus:ring-primary/20 text-sm font-medium"/>
                        </Card>
                    </motion.div>
                </div>
            </div>
