...
return (<motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="col-span-12 px-2 pb-2">
            <div className="bg-app rounded-xl p-2 shadow-neu-inset border border-primary/10">
                <div className="flex items-center gap-3">
                    <div className="flex items-center gap-1">
                        {/* Stop propagation so pressing timer controls does not toggle parent rows. */}
                        <button onClick={(e) => { e.stopPropagation(); isRestTimerPaused ? resumeRestTimer() : pauseRestTimer(); }} className="w-8 h-8 rounded-full bg-neu shadow-neu-sm flex items-center justify-center text-primary hover:brightness-125 transition-all">
                            {isRestTimerPaused ? <Play size={14} fill="currentColor"/> : <Pause size={14} fill="currentColor"/>}
                        </button>
                        <button onClick={(e) => { e.stopPropagation(); stopRestTimer(); }} className="w-8 h-8 rounded-full bg-neu shadow-neu-sm flex items-center justify-center text-text-muted hover:text-primary transition-all">
                            <SkipForward size={14} fill="currentColor"/>
                        </button>
                    </div>

                    <div className="flex-1 relative h-2.5 bg-divider/10 rounded-full overflow-hidden shadow-neu-inset">
                        <motion.div className="absolute inset-y-0 left-0 bg-primary shadow-[0_0_15px_rgba(0,0,238,0.4)]" initial={{ width: "100%" }} animate={{ width: `${progress * 100}%` }} transition={{ duration: 0.5, ease: "linear" }}/>
                    </div>

                    <div className="text-[10px] font-black text-primary tabular-nums w-10 text-right uppercase tracking-[0.1em]">
                        {timeLeft}s
                    </div>
                </div>
            </div>
