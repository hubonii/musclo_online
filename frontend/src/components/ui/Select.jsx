...
export default function Select({ value, onValueChange, options, placeholder = 'Select...', label, className, variant = 'default' }) {
    ...
    const triggerVariants = {
        ...
        'neu-inset': 'bg-surface shadow-neu-inset border-none focus-within:ring-1 focus-within:ring-primary/30 rounded-xl',
        'neu-flat': 'bg-surface shadow-neu border-none hover:shadow-neu-sm transition-colors rounded-xl',
        'ghost-primary': 'bg-transparent text-primary font-black border-none hover:bg-primary/5 px-1 h-auto rounded-xl',
...
return (<div className={className}>
            ...
            <RadixSelect.Root value={value} onValueChange={onValueChange}>
                <RadixSelect.Trigger className={cn('inline-flex items-center justify-between w-full h-[46px] px-4 transition-all outline-none', 'text-text-primary text-sm font-black uppercase tracking-tight', triggerVariants[variant] || triggerVariants.default, 'data-[placeholder]:text-text-muted')}>
                    <RadixSelect.Value placeholder={placeholder}/>
                    <RadixSelect.Icon>
                        <ChevronDown size={variant === 'compact' ? 12 : 16} className={cn(variant === 'ghost-primary' ? "text-primary" : "text-text-muted", "ml-1")}/>
                    </RadixSelect.Icon>
                </RadixSelect.Trigger>

                <RadixSelect.Portal>
                    <RadixSelect.Content className={cn('z-[100] overflow-hidden rounded-2xl bg-surface border border-primary/10 shadow-2xl p-1.5', 'animate-in fade-in-0 zoom-in-95 duration-100 ease-out fill-mode-forwards', 'shadow-neu-primary/10')} position="popper" sideOffset={8}>
                        <RadixSelect.Viewport className="min-w-[160px]">
                            {options.map((option) => (<RadixSelect.Item key={option.value} value={option.value} className={cn('relative flex items-center gap-3 px-4 py-3 text-xs font-black uppercase tracking-widest rounded-xl cursor-pointer outline-none mb-0.5 last:mb-0', 'text-text-secondary transition-all', 'hover:bg-primary/5 hover:text-primary', 'focus:bg-primary/5 focus:text-primary focus:pl-5', 'data-[state=checked]:bg-primary/10 data-[state=checked]:text-primary')}>
