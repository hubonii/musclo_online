...
return (<div className="w-full">
                ...
                <div className="relative">
                    ...
                    <input 
                        ref={ref} 
                        id={inputId} 
                        className={cn('w-full h-12 px-5 rounded-xl !bg-app !text-text-primary text-base shadow-neu-inset font-black', 'placeholder:text-text-muted', 'focus:outline-none focus:shadow-neu-inset-focused focus:!bg-app focus:!text-text-primary', 'disabled:opacity-50 disabled:cursor-not-allowed', 'transition-colors duration-100', icon && 'pl-12', suffix && 'pr-12', error && 'ring-2 ring-danger', className)} 
                        aria-invalid={!!error}
                ...
                {error && (<p id={`${inputId}-error`} className="mt-1 text-xs text-danger animate-pulse">{error}</p>)}
            </div>);
});
