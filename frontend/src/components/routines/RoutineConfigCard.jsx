...
return (<div className="bg-surface p-6 rounded-2xl shadow-neu flex flex-col gap-4">
            <input type="text" placeholder="Routine Name" value={routineName} onChange={(e) => setRoutineName(e.target.value)} className="bg-app px-4 py-4 rounded-xl text-xl font-black text-text-primary border-none outline-none focus:ring-2 focus:ring-primary/30 transition-all shadow-neu-inset placeholder:text-text-muted font-display uppercase tracking-tight"/>
            ...
            <div className="flex bg-neu shadow-neu-inset rounded-2xl p-1 gap-1 overflow-x-auto scrollbar-hide shrink-0">
                ...
                ? 'bg-primary text-white shadow-neu-primary border-t border-white/20'
                : 'text-text-muted hover:text-primary hover:bg-app cursor-pointer'}`}>
