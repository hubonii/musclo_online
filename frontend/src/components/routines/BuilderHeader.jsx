...
return (<div className="sticky top-0 z-40 bg-surface w-full shadow-neu-sm p-3 md:p-4 flex items-center justify-between overflow-x-hidden safe-area-top">
            <div className="flex items-center gap-1.5 md:gap-2 text-sm font-semibold tracking-wide min-w-0">
                <div className="hidden md:flex items-center gap-2">
                    <span className="text-text-muted cursor-pointer hover:text-primary transition-colors shrink-0" onClick={() => onNavigate('/programs')}>
                        Programs
                    </span>
                    <ChevronRight size={16} className="text-divider/40 shrink-0"/>
                    <span className="text-text-muted cursor-pointer hover:text-primary transition-colors truncate max-w-[150px]" onClick={() => onNavigate(programId === 'standalone' ? '/programs' : `/programs/${programId}`)}>
            ...
            <div className="flex gap-2 shrink-0">
                ...
                <Button variant="primary" onClick={onSave} disabled={isSaving} className="px-4 md:px-8 font-black uppercase tracking-widest shadow-neu-primary/10 text-[10px] md:text-xs">
