...
return (<nav className="md:hidden fixed bottom-0 inset-x-0 z-40 bg-neu pb-[env(safe-area-inset-bottom,0px)] h-[calc(3.75rem+env(safe-area-inset-bottom,0px))] ">
            <div className="flex items-center justify-around h-full px-4">
                ...
                {NAV_ITEMS.map((item) => (<NavLink key={item.path} to={item.path} className={({ isActive }) => cn('relative flex items-center justify-center w-full h-full transition-colors duration-100', isActive ? 'text-primary' : 'text-text-secondary')}>
                        {({ isActive }) => (<>
                        ...
                        ? 'bg-neu shadow-neu-inset text-primary scale-105'
